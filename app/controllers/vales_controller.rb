class ValesController < ApplicationController
  layout "layouts/vale"
  before_action :set_vale, only: [:show, :edit, :editpdf, :update, :destroy,:aprobar,:pdf,:text,:cancelar]

  # GET /credits
  # GET /credits.json
  def pdf
    send_data Base64.decode64(@vale.pdf64[28..@vale.pdf64.length]), :type => 'application/pdf', :filename => "documentos.pdf", :disposition => "inline"
  end
  def text
    render text:@vale.pdf64
  end
  def index
    @vales = Vale.all.where("status=1").select(:fecha_de_contrato,:id,:fecha,:apellido_paterno,:apellido_materno,:nombre_1,:nombre_2,:RFC,:fecha_de_contrato,:monto_solicitud,:agente_empresa,:referencia_agente_empresa)
    nom1 = params[:nombre1]
    nom2 = params[:nombre2]
    pat = params[:paterno]
    mat = params[:materno]
    ...
    printed = params[:printed]
     @vales =@credits.where("lower(CURP) = '#{curp.downcase}'") unless  params[:curp].nil? or  params[:curp]==""
     @vales =@credits.where("lower(nombre_1) like '#{nom1.downcase}%'") unless  params[:nombre1].nil? or  params[:nombre1]==""
     @vales =@credits.where("lower(nombre_2) like '#{nom2.downcase}%'") unless  params[:nombre2].nil? or  params[:nombre2]==""
     @vales =@credits.where("lower(apellido_paterno) like '#{pat.downcase}%'") unless  params[:paterno].nil? or  params[:paterno]==""
     @vales =@credits.where("lower(apellido_materno) like '#{mat.downcase}%'") unless  params[:materno].nil? or  params[:materno]==""
     
     @vales =@credits.where(:fecha_de_contrato => fecha1.to_date.beginning_of_day..fecha2.to_date.end_of_day) unless params[:fecha1].nil? or params[:fecha1]=="" or params[:fecha2].nil? or params[:fecha2]==""
     @vales =@credits.where("credits.agente_empresa = ? and credits.referencia_agente_empresa = ? ",tipo_padre,padre_id) unless  params[:tpadre].nil? or  params[:tpadre]=="" or  params[:padre].nil? or  params[:padre]==""
     @vales =@credits.where("credits.product_id = ?  ",product_id)  unless  params[:product_id].nil? or  params[:product_id]==""
     @vales =@credits.where(fecha_de_contrato:nil) unless  params[:printed].nil? or  params[:printed]==""
    
     @vales = @credits.order(:created_at)
     
     @vales = Vale.get_by_branch_office(@vales,BranchOffice.find(branch_office_id.to_i)) unless  params[:sucursal_id].nil? or  params[:sucursal_id]==""
     
     @vales = Vale.get_by_branch_office(@vales,current_user.branchOffices[0]) if (current_user.tipo==3)
  ...
  def cancelar
    ...
    respond_to do |format|
      format.html { redirect_to customer_historial_path(@vale.customer), notice: 'El credito se ha cancelado correctamente' }
    end
  end
  ...
  def create
    ...
    respond_to do |format|
      if @vale.save
        format.html { redirect_to @vale, notice: 'Credit was successfully created.' }
